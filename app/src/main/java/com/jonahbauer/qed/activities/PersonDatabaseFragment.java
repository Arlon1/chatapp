...
public class PersonDatabaseFragment extends Fragment implements CompoundButton.OnCheckedChangeListener, QEDDBLoginReceiver, QEDDBPersonsListReceiver {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        personAdapter = new PersonAdapter(getContext(), new ArrayList<>(), person -> person.firstName.charAt(0), Comparator.comparing(person -> person.firstName + " " + person.lastName), view.findViewById(R.id.fixed_header));
    ...
    public void search() {
        ...
        if (sortLastName) {
            personAdapter.setComparator(Comparator.comparing(person -> person.lastName + ", " + person.firstName));
            personAdapter.setHeaderMap(person -> person.lastName.charAt(0));
        } else {
            personAdapter.setComparator(Comparator.comparing(person -> person.firstName + " " + person.lastName));
            personAdapter.setHeaderMap(person -> person.firstName.charAt(0));
        }
